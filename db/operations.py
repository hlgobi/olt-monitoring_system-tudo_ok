...
# ==============================================================================
# MÓDULO DE OPERAÇÕES NO BANCO DE DADOS
# ==============================================================================
# Este arquivo implementa funções para operações no banco de dados PostgreSQL relacionadas ao monitoramento de OLTs e ONTs.
# Ele é responsável pela persistência de dados coletados das OLTs, incluindo informações de ONTs, status de PONs,
# dados de temperatura e recursos, além de histórico de diagnósticos.
#
# As funções deste módulo garantem a integridade dos dados, tratam conversões de tipos, gerenciam conexões e emitem sinais para atualização da interface
# gráfica quando necessário.

# ==============================================================================
# IMPORTAÇÕES DE MÓDULOS
# ==============================================================================
from datetime import datetime, timezone, timedelta  # Manipulação de datas e horas
...
from psycopg2.extras import execute_batch  # Para operações em lote no PostgreSQL
from PyQt5.QtCore import QObject, pyqtSignal  # Para emissão de sinais PyQt

# Importações específicas da aplicação
from config import DB_CONFIG  # Configurações do banco de dados
from gui.signals import db_signals  # Sinais para comunicação com a GUI
from utils.helpers import parse_descricao_avancada  # Função para parsing de descrições


# ==============================================================================
# FUNÇÕES AUXILIARES PARA MANIPULAÇÃO DE DATA/HORA
# ==============================================================================
...
def get_current_brasilia_time():
    ...
    Esta função é utilizada para garantir que todos os registros de data/hora
    no banco de dados estejam consistentes com o fuso horário de Brasília,
    independentemente do fuso horário do servidor onde a aplicação está rodando.
    
    Returns:
        datetime: Objeto datetime com o horário atual de Brasília e informação de fuso
    """
...
def to_db_timestamp(ts_str):
    """
    Converte string de timestamp para objeto datetime com fuso horário de Brasília.
    
    Esta função trata múltiplos formatos de string de data/hora e os converte
    para objetos datetime com o fuso horário de Brasília corretamente configurado.
    É utilizada para garantir que dados de timestamp vindos das OLTs sejam
    armazenados corretamente no banco de dados.
    
    Args:
        ts_str (str): String representando um timestamp em diversos formatos possíveis
        
    Returns:
        datetime or None: Objeto datetime com fuso de Brasília ou None se conversão falhar
    """
...
def get_existing_ont_data(olt_ip, serial_number):
    ...
    Esta função consulta o registro mais recente de uma ONT específica para obter
    informações como o código de conexão e o nome do cliente, que podem ser
    preservados em atualizações subsequentes.
    
    Args:
        olt_ip (str): Endereço IP da OLT onde a ONT está conectada
        serial_number (str): Número de série da ONT
        
    Returns:
        dict or None: Dicionário com os dados da ONT ou None se não encontrado
    """
    try:
        conn = psycopg2.connect(**DB_CONFIG)
        cursor = conn.cursor()
...
def get_paginated_ont_data(olt_ip=None, limit=1000, offset=0):
    """
    Busca dados de ONTs com paginação para evitar travamentos em grandes volumes de dados.
    
    Esta função implementa paginação para consultas de dados de ONTs, permitindo
    que a aplicação trabalhe com conjuntos de dados gerenciáveis em vez de carregar
    todos os registros de uma vez, o que poderia causar problemas de performance.
    
    Args:
        olt_ip (str, optional): Filtra por OLT específica. Se None, busca todas
        limit (int): Número máximo de registros a retornar
        offset (int): Número de registros a pular (para paginação)
        
    Returns:
        list: Lista de tuplas com os dados das ONTs encontradas
    """
    try:
        conn = psycopg2.connect(**DB_CONFIG)
        cursor = conn.cursor()
...
# ==============================================================================
# FUNÇÕES PARA PERSISTÊNCIA DE DADOS DE PON
# ==============================================================================
...
def save_pon_status(olt_ip, fsp, online_count, total_count):
    ...
    Esta função registra o status atual de uma interface PON, incluindo o número
    de ONTs online e o total de ONTs. O status é determinado automaticamente
    com base na proporção de ONTs online:
    - "empty": Nenhuma ONT conectada
    - "operational": Todas as ONTs online
    - "degraded": Pelo menos 70% das ONTs online
    - "faulty": Menos de 70% das ONTs online
...
def save_pon_traffic_data(olt_ip, fsp, traffic_data):
    ...
    Esta função armazena informações de tráfego de uma interface PON, incluindo
    tráfego de upload/download em kbps e pacotes por segundo por categoria
    (unicast, multicast, broadcast) para ambas as direções.
    
    Args:
        olt_ip (str): Endereço IP da OLT
        fsp (str): Identificador Frame/Slot/Porta da PON
        traffic_data (dict): Dicionário com os dados de tráfego
...
def save_pon_port_state(olt_ip, fsp, state_data):
    ...
    Esta função armazena informações detalhadas sobre o estado operacional de uma
    porta PON, incluindo estado atual, causa da última queda, tempos de atividade,
    detecção de sinal, status do módulo óptico e diversos parâmetros operacionais.
    
    Args:
        olt_ip (str): Endereço IP da OLT
        fsp (str): Identificador Frame/Slot/Porta da PON
        state_data (dict): Dicionário com os dados de estado da porta
...
def save_pon_statistics_packets(olt_ip, fsp, stats_data):
    ...
    Esta função armazena estatísticas detalhadas de tráfego de pacotes de uma
    interface PON, incluindo contadores de frames e bytes recebidos/enviados,
    frames por tamanho, frames descartados e frames com erro.
    
    Args:
        olt_ip (str): Endereço IP da OLT
        fsp (str): Identificador Frame/Slot/Porta da PON
        stats_data (dict): Dicionário com as estatísticas de pacotes
    ...
    try:
        ...
        # Define as colunas e valores para a inserção
        columns = [
            'olt_ip', 'olt_identifier', 'fsp',
...
# ==============================================================================
# FUNÇÕES PARA PERSISTÊNCIA DE DADOS DE MONITORAMENTO
# ==============================================================================
...
def save_temp_data(olt_ip, temp_data):
    ...
    Esta função armazena informações de temperatura dos slots das OLTs,
    incluindo temperatura em Celsius e Fahrenheit, e o status operacional.
    
    Args:
        olt_ip (str): Endereço IP da OLT
        temp_data (list): Lista de dicionários com dados de temperatura por slot
        
    Returns:
        int: Número de registros salvos com sucesso, ou 0 em caso de falha
    """
...
def save_resource_data(olt_ip, resource_data):
    ...
    Esta função armazena informações de utilização de recursos (CPU, memória)
    dos slots das OLTs, incluindo percentual de uso e status operacional.
    
    Args:
        olt_ip (str): Endereço IP da OLT
        resource_data (list): Lista de dicionários com dados de recursos por slot
...
def save_uplink_ddm_data(olt_ip, ddm_data_list):
    """
    Salva dados DDM (Digital Diagnostics Monitoring) de uplink no banco de dados.
    
    Esta função armazena informações de monitoramento digital de diagnóstico
    das portas de uplink, incluindo temperatura, tensão, corrente de polarização,
    potências ópticas e status operacional.
    
    Args:
        olt_ip (str): Endereço IP da OLT
        ddm_data_list (list): Lista de dicionários com dados DDM por porta
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            # Usa execute_batch para melhor performance em inserções múltiplas
...
# ==============================================================================
# FUNÇÕES PARA PERSISTÊNCIA DE DADOS DE TRÁFEGO DE ONT
# ==============================================================================
...
def save_ont_traffic_bulk(olt_ip, fsp, traffic_list):
    ...
    Esta função utiliza inserção em lote para otimizar o desempenho ao salvar
    múltiplos registros de tráfego de ONTs de uma vez. Cada registro contém
    informações de tráfego de upload e download em kbps para uma ONT específica.
    
    Args:
        olt_ip (str): Endereço IP da OLT
        fsp (str): Identificador Frame/Slot/Porta da PON
        traffic_list (list): Lista de dicionários com dados de tráfego por ONT
        
    Returns:
        int: Número de registros salvos com sucesso, ou 0 em caso de falha
        
    Raises:
        Exception: Propaga exceções para tratamento pelo chamador
    """
    if not traffic_list:
        return 0
    ...
    try:
        ...
        if conn:
            conn.rollback()
        raise  # Força o erro a ser reportado para a GUI
    finally:
        if conn:
            conn.close()
...
def save_ont_statistics_packets_bulk(olt_ip, fsp, stats_list):
    ...
    Esta função utiliza inserção em lote para otimizar o desempenho ao salvar
    múltiplos registros de estatísticas de pacotes de ONTs. Cada registro contém
    contadores de frames e bytes para upstream e downstream.
    
    Args:
        olt_ip (str): Endereço IP da OLT
        fsp (str): Identificador Frame/Slot/Porta da PON
        stats_list (list): Lista de dicionários com estatísticas por ONT
...
def save_ont_eth_statistics_bulk(olt_ip, fsp, ont_id, stats_list_per_port):
    ...
    Esta função utiliza inserção em lote para otimizar o desempenho ao salvar
    múltiplos registros de estatísticas de interfaces Ethernet de uma ONT.
    Cada registro contém contadores de frames, bytes e erros por porta Ethernet.
    
    Args:
        olt_ip (str): Endereço IP da OLT
        fsp (str): Identificador Frame/Slot/Porta da PON
        ont_id (int): ID da ONT na PON
        stats_list_per_port (list): Lista de dicionários com estatísticas por porta Ethernet
...
# ==============================================================================
# FUNÇÕES PARA GERENCIAMENTO DE HISTÓRICO DE DIAGNÓSTICO
# ==============================================================================
...
def get_ont_diagnostic_history(ont_serial_number: str, olt_identifier: str = None):
    ...
    Esta função consulta o banco de dados por registros de diagnóstico de uma ONT,
    podendo filtrar também por OLT se especificado. Retorna os registros mais
    recentes primeiro, limitado a 500 registros para evitar sobrecarga.
...
def save_ont_diagnostic_data(diag_info: dict):
    ...
    Esta função armazena informações detalhadas de diagnóstico de uma ONT,
    incluindo saída bruta de comandos, dados parseados, informações do fabricante,
    modelo, uptime e versão de firmware.
    
    Args:
        diag_info (dict): Dicionário contendo todas as informações de diagnóstico
    """
    conn = None
